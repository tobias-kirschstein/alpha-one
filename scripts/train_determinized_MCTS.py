...
n_iterations = 5  # How often the whole procedure is repeated. Also corresponds to the number of evaluations

# Train samples generation
n_games_train = 50  # How many new states will be generated by the best model via self-play for training (Training set size delta). Has to be larger than batch_size
n_games_valid = 50

# Model update
n_most_recent_train_samples = 10  # Among which training samples to choose to train current model
n_most_recent_valid_samples = 10
n_train_steps = 10  # After how many gradient updates the new model tries to beat the current best
n_valid_steps = 5
batch_size = 5
