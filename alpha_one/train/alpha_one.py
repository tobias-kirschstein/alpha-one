...
def _generate_one_game_parallel(game, checkpoint_manager: Dict[str, CheckpointManager], mcts_config: MCTSConfig):
    # Tensorflow models are not pickeable. Hence, the worker has to load the model from the disk
    
    model_current_best = [checkpoint_manager["observation_model_manager"].load_checkpoint(-1), checkpoint_manager["game_model_manager"].load_checkpoint(-1)]
...
class AlphaOneTrainManager:
    ...
    def generate_training_data(self, n_games_train: int, n_games_valid: int, mcts_config: MCTSConfig):
        ...
        for _ in range(n_games_train):
            if self.use_parallelism:
                ...
                observation_samples, game_samples = _generate_one_game_parallel.remote(game=self.game, checkpoint_manager=self.checkpoint_manager,
                                                                                       mcts_config=mcts_config)
                train_observation_samples.append(observation_samples)
                train_game_samples.append(game_samples)
            else:
                observation_samples, game_samples = _generate_one_game(game=self.game, model_current_best=self.model_current_best,
                                       mcts_config=mcts_config)
    ...
    def train_model(self, n_train_steps, n_valid_steps, batch_size, weight_decay):
        ...
        for _ in range(n_valid_steps):
            ...
            policy_loss = np.mean(cross_entropy(np.array([sample.policy for sample in valid_observation_samples]), policies))
    ...
    def replace_model_with_challenger(self, challenger_win_rate: float, win_ratio_needed: float):
        if challenger_win_rate > win_ratio_needed:
            self.checkpoint_manager["observation_model_manager"].store_checkpoint(self.model_challenger[0], self.get_player_name_challenger())
            self.checkpoint_manager["game_model_manager"].store_checkpoint(self.model_challenger[1], self.get_player_name_challenger())
            
            self.model_current_best = [self.checkpoint_manager["observation_model_manager"].load_checkpoint(self.get_player_name_challenger()),\
